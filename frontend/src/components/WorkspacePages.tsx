...
import type { Agent, ChatMessage, CVSceneResult, CVTrainedResult, SimulationResult, Strategy, WorldEvent, WorldState, Zone } from '../types'
...
export function EventsPage({ world, agents, resetVersion, onComplete, onSelectEvent }: { world: WorldState; agents: Agent[]; resetVersion: number; onComplete: (result: CVSceneResult | CVTrainedResult) => void; onSelectEvent: (event: WorldEvent) => void }) {
  return (
...
export function LabPage({ resetVersion, results, strategy, busy, onRun, onCompare, onViewResult, currentRisk, riskHistory, messages, onSend, onViewTrace }: { resetVersion: number; results: SimulationResult[]; strategy: Strategy; busy: boolean; onRun: (strategy: Strategy) => void; onCompare: () => void; onViewResult: (result: SimulationResult) => void; currentRisk: number; riskHistory: number[]; messages: ChatMessage[]; onSend: (message: string) => void; onViewTrace: (message: ChatMessage) => void }) {
  return (
    <div className="secondary-page">
      ...
      <div className="lab-layout"><div className="lab-primary"><TrainedModelsPanel resetVersion={resetVersion} /><SimulationPanel results={results} selected={strategy} busy={busy} onRun={onRun} onCompare={onCompare} onViewResult={onViewResult} /><PredictionPanel currentRisk={currentRisk} resetVersion={resetVersion} /><section className="panel chart-panel"><div className="panel-heading"><span>风险演化</span><em>风险规则</em></div><RiskChart values={riskHistory} /></section></div><ChatPanel messages={messages} busy={busy} onSend={onSend} onViewTrace={onViewTrace} /></div>
    </div>
  )
}
