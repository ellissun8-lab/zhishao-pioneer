...
高德地图采用 **JS API 2.0 + `@amap/amap-jsapi-loader`**。开发环境使用 Web 端（JS API）Key 与安全密钥；2021-12-02 之后申请的 JS API Key 需要配合安全密钥使用。
...
```env
VITE_AMAP_KEY=这里填写高德Web端JSAPI_KEY
VITE_AMAP_SECURITY_JS_CODE=这里填写高德安全密钥
```

真实 Key 与安全密钥只允许存在于开发者本机环境变量或部署平台 Secret 中，不得写入源码、测试代码、README、截图或提交到 Git。
...
`frontend/src/map/amap.ts` 统一负责加载高德 JS API，禁止在多个 React 组件重复调用 Loader。
...
如 TypeScript 缺少 `_AMapSecurityConfig` 声明，在项目类型声明文件中补充：
...
new AMap.Map(...)
↓
创建 Marker / Polyline / Circle / Polygon
↓
业务状态变化时只更新覆盖物
↓
组件卸载时 map.destroy()
...
禁止 React 每次 render 都重新创建地图实例，避免重复实例、内存泄漏和覆盖物重复。
...
地图中心点与 Demo 城市通过配置文件管理，不允许散落硬编码在多个组件中，例如：
...
Demo 城市统一固定为**广州市（Guangzhou）**。所有 Agent 初始位置、历史轨迹、预测轨迹、敏感区域和 Demo Event 坐标均应分布在广州地图范围内，并明确标记为 Synthetic Data。禁止继续使用其他城市的历史 Demo 坐标。

可优先围绕广州中心城区构造演示场景，例如天河、越秀、海珠等区域；但人员身份、关系、轨迹、风险等级和风险事件仍全部使用合成数据，不得表述为真实广州人员或真实城市事件。
...
并继续支持 Agent、风险区域、轨迹、事件和 What-if 的演示，保证黑客松现场不因地图网络或鉴权问题阻塞核心 Demo。
...
```text
✓ VITE_AMAP_KEY 能被 Vite 读取
✓ VITE_AMAP_SECURITY_JS_CODE 能被读取
✓ AMapLoader.load(version="2.0") 成功
✓ 地图容器有明确高度
✓ 地图实例只创建一次
✓ Marker 正常显示
✓ Polyline 正常显示
...
开发环境可使用前端静态安全密钥方式；如部署到公开环境，优先按高德官方安全建议改为服务端代理方式保护安全密钥。
...
地图初始化后应直接定位广州市范围，核心演示流程全部在广州地图上完成。
