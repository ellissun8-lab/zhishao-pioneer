...
def write_streaming(episodes: int, seed: int, weight: float, out_dir: Path, batch_size: int) -> dict[str, object]:
    ...
    try:
        for record in iter_episode_records(episodes, seed, weight, runtime_stats=runtime_stats):
            stats.update(record)
...
## Feature Uniqueness
- duplicate_feature_rows: {result["runtime_stats"].get("duplicate_feature_rows")}
...
def main() -> None:
    ...
    summary = {
        ...
        "duplicate_feature_rows": result["runtime_stats"].get("duplicate_feature_rows"),
        "feature_resample_count": result["runtime_stats"].get("feature_resample_count", 0),
    ...
    print(f"duplicate_feature_rows={summary['duplicate_feature_rows']} (resamples={summary['feature_resample_count']})")
